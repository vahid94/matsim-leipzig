...
public class ActivitiesOnPtLinkAnalysis {

    Map<Id<Link>, String> ptLinksWithActivities = new HashMap<>();
    Map<Id<Link>, String> followingLegs = new HashMap<>();
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Activites on pt links written to " + outputFile);
    }
                    ...
                    if(((Activity) element).getLinkId().toString().contains("pt")) {
                        ...
                        if(!(planElements.indexOf(element) >= planElements.size() - 1)) {
                            if(planElements.get(planElements.indexOf(element) + 1) instanceof Leg) {
                                ...
                                new RuntimeException("PlanElement " + element + " is an Activity but is not followed by a Leg!");
                            }
                        } else {
