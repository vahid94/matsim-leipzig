...
public class RunLeipzigIntegrationTest {
	...
	public final void runPoint1pctIntegrationTest() {
		...
		Config config = ConfigUtils.loadConfig("input/v1.2/leipzig-v1.2-25pct.noDRT.config.xml");
...
//				"--parking-cost-area", exampleShp);
		...
		MATSimApplication.execute(RunLeipzigScenario.class, config, "run", "--1pct", "--slow-speed-area", exampleShp,
				"--slow-speed-relative-change", "0.5","--drt-area", exampleShp, "--drt-modes", "drtNorth,drtSoutheast", "--drt-case", "twoSeparateServiceAreas", "--post-processing", "disabled",
				"--parking-cost-area", exampleShp);
